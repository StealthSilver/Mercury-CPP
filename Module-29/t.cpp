// MODULE 29 — Problem 3: Kth smallest in BST (LeetCode 230)

#include <iostream>
using namespace std;

struct Node {
    int data;
    Node* left;
    Node* right;
    Node(int v) : data(v), left(nullptr), right(nullptr) {}
};

Node* insert(Node* root, int key) {
    if (!root) {
        return new Node(key);
    }
    if (key < root->data) {
        root->left = insert(root->left, key);
    } else if (key > root->data) {
        root->right = insert(root->right, key);
    }
    return root;
}

void kthSmallest(Node* root, int& k, int& ans) {
    if (!root || k == 0) {
        return;
    }
    kthSmallest(root->left, k, ans);
    if (k == 0) {
        return;
    }
    if (--k == 0) {
        ans = root->data;
        return;
    }
    kthSmallest(root->right, k, ans);
}

void freeTree(Node* root) {
    if (!root) {
        return;
    }
    freeTree(root->left);
    freeTree(root->right);
    delete root;
}

int main() {
    cout << "Problem 3: Kth smallest in BST (1-indexed)\n";
    cout << "=========================================\n\n";

    Node* root = nullptr;
    for (int x : {8, 3, 10, 1, 6, 14, 7, 13}) {
        root = insert(root, x);
    }

    cout << "BST inorder: 1 3 6 7 8 10 13 14\n\n";

    for (int k : {1, 3, 5, 8}) {
        int kk = k;
        int ans = -1;
        kthSmallest(root, kk, ans);
        cout << "k = " << k << "  ->  kth smallest value = " << ans << "\n";
    }

    cout << "\nTime: O(h + k)   Space: O(h)\n";

    freeTree(root);
    return 0;
}
